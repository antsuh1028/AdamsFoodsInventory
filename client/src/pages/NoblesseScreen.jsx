...
const Th = ({ children, ...props }) => (
  <Box as="th" px={2} py={1.5} textAlign="left"
    ...
    <Box>
      ...
      {open && (
        <Flex flexWrap="wrap" gap={1} maxW="180px" mt={1}>
          {boxes.map((b, k) => (
            <Box key={k} fontSize="xs" color="gray.600" bg="gray.100" borderRadius="sm"
              px={1.5} py={0.5} minW="48px" textAlign="center">
              {b.weight} lb
            </Box>
          ))}
        </Flex>
          ...
          {orders.map((order, i) => (
            <React.Fragment key={order.id}>
              <Box as="tr" bg={i % 2 === 0 ? "white" : "gray.50"}
                ...
                <Td color="gray.400" fontSize="xs" py={1}>{expandedId === order.id ? "▾" : "▸"}</Td>
                <Td fontWeight="medium" py={1}>{fmtDate(order.sentDate)}</Td>
                <Td fontSize="xs" color="gray.500" maxW="160px" overflow="hidden" textOverflow="ellipsis" whiteSpace="nowrap" py={1}>
                  {itemsMap[order.id]
                    ? itemsMap[order.id].map((it) => it.description).filter(Boolean).join(", ") || "—"
                    : <Text as="span" fontSize="xs" color="gray.300">expand to load</Text>
                  }
                </Td>
                <Td py={1}>{order.itemCount}</Td>
                <Td py={1}>{order.totalWeight ? `${parseFloat(order.totalWeight).toFixed(0)} lb` : "—"}</Td>
              </Box>
              {expandedId === order.id && (
                <Box as="tr">
...
const PendingOrdersTab = ({ orders }) => {
  ...
  const cutoff    = rangeDays != null ? daysAgo(rangeDays) : null;
  const filtered  = cutoff ? orders.filter((o) => o.sentDate >= cutoff) : orders;
  const todayOrders = filtered.filter((o) => o.sentDate === todayStr);
  const prevOrders  = filtered.filter((o) => o.sentDate <  todayStr);
  const hiddenCount = orders.length - filtered.length;

  return (
    <Box>
      {/* Range filter */}
      <Flex gap={1} mb={4} align="center" flexWrap="wrap">
        <Text fontSize="xs" color="gray.400" mr={1}>Show:</Text>
        {RANGE_OPTIONS.map((opt) => (
          <Button
            key={opt.label}
            size="xs"
            borderRadius="full"
            variant={rangeDays === opt.days ? "solid" : "outline"}
            colorScheme="orange"
        ...
        {hiddenCount > 0 && (
          <Text fontSize="xs" color="gray.400" ml={2}>
            +{hiddenCount} older order{hiddenCount !== 1 ? "s" : ""} hidden
          </Text>
        )}
      </Flex>
      ...
      {filtered.length === 0 && (
        <Text fontSize="sm" color="gray.400">No orders in this range.</Text>
      )}
    </Box>
  );
};
              ...
              <Box as="tr" key={`${r.id}-${j}`}
                ...
                {j === 0 ? (
                  <>
                    ...
                    <Td rowSpan={r.lines.length} borderLeft="1px" borderColor="gray.100" verticalAlign="top">
                      <Select size="xs" value={r.status} borderRadius="md" w="130px"
                        bg={`${STATUS_COLORS[r.status]}.50`} color={`${STATUS_COLORS[r.status]}.700`}
                        borderColor={`${STATUS_COLORS[r.status]}.200`}
                        onChange={(e) => onStatusChange(r.id, e.target.value)}>
...
const IncomingRecordsTab = ({ pendingOrders, receipts, onReceiptAdded, onStatusChange }) => {
  ...
  const handleSubmit = async () => {
    if (!shipmentDate || lines.every((l) => !l.lot && !l.brand)) {
    ...
    try {
      const receipt = await axiosInstance.post("/noblesse-receipts", {
        ...
        lines: lines.filter((l) => l.lot || l.brand),
      });
      toast({ title: "Shipment logged", status: "success", position: "top", duration: 2500, isClosable: true });
      onReceiptAdded(receipt.data);
      setShipmentDate(today()); setBolNumber(""); setDriver(""); setLinkedOrderId("");
      setLines([emptyLine()]); setShowForm(false);
    } catch {
  ...
  return (
    <Box>
      <Flex justify="space-between" align="center" mb={4}>
        ...
        <Button size="xs" leftIcon={<AddIcon />} colorScheme="orange" variant={showForm ? "solid" : "outline"}
      ...
      {showForm && (
        <Box bg="orange.50" border="1px" borderColor="orange.200" borderRadius="lg" p={4} mb={6}>
          ...
          <Box mb={4}>
            ...
            <Select size="sm" placeholder="— select order —" value={linkedOrderId}
              onChange={(e) => setLinkedOrderId(e.target.value)} bg="white" borderRadius="md" maxW="360px">
              {pendingOrders.map((o) => (
  ...
  const ReceiptCard = ({ receipt, accentColor }) => (
    <Box border="1px" borderColor={`${accentColor}.200`} borderRadius="lg" mb={4} overflow="hidden">
      <Flex bg={`${accentColor}.50`} px={4} py={3} justify="space-between" align="center">
        ...
        <Text fontSize="xs" color="gray.400">{fmtDate(receipt.createdAt)}</Text>
      </Flex>
      <Box p={4}>
          ...
          : (
            <Box overflowX="auto">
              <Box as="table" width="100%" borderCollapse="collapse">
                <thead>
                  <tr>
                    <Th>Lot #</Th><Th>Brand</Th><Th>Grade</Th>
                    <Th>Weight</Th><Th>Qty</Th><Th>Date Received</Th>
                  </tr>
                </thead>
                <tbody>
...
const NtiInventoryTab = ({ ntiInventory, afItems, onAdd, onUpdate, onDelete }) => {
  ...
  const buildDiscrepancies = () => {
    const afByLot  = {};
    const ntiByLot = {};
    afItems.forEach((item) => { const k = (item.lot || "").trim().toLowerCase(); if (k) afByLot[k] = item; });
    ntiInventory.forEach((item) => { const k = (item.lot || "").trim().toLowerCase(); if (k) ntiByLot[k] = item; });
    ...
    afItems.forEach((af) => {
      ...
      const weightDiff   = nti && af.weight != null && nti.weight != null ? Math.abs(Number(af.weight) - Number(nti.weight)) > 0.5 : false;
      const descMismatch = nti && af.description && nti.description ? af.description.trim().toLowerCase() !== nti.description.trim().toLowerCase() : false;
      const brandMismatch = nti && af.brand && nti.brand ? af.brand.trim().toLowerCase() !== nti.brand.trim().toLowerCase() : false;
      rows.push({ lot: af.lot, afDesc: af.description, ntiDesc: nti?.description, afBrand: af.brand, ntiBrand: nti?.brand,
        afWeight: af.weight, ntiWeight: nti?.weight, matched: !!nti, weightDiff, descMismatch, brandMismatch,
    ...
    ntiInventory.forEach((nti) => {
      ...
      rows.push({ lot: nti.lot, afDesc: null, ntiDesc: nti.description, afBrand: null, ntiBrand: nti.brand,
        afWeight: null, ntiWeight: nti.weight, matched: false, weightDiff: false, descMismatch: false, brandMismatch: false,
        ntiOnly: true, hasIssue: true });
    });
    return rows;
  };
  ...
  return (
    <Box>
      <Flex justify="space-between" align="center" mb={4} flexWrap="wrap" gap={2}>
        <Text fontSize="xs" fontWeight="semibold" color="gray.600" textTransform="uppercase" letterSpacing="wide">NTI Inventory</Text>
        <Flex gap={2}>
          <Button size="xs" variant="outline" colorScheme={showCompare ? "red" : "purple"}
            onClick={() => setShowCompare((v) => !v)}>
            {showCompare ? "Hide Compare" : "Compare with AF Orders"}
            {!showCompare && afItems.length > 0 && <Badge ml={2} colorScheme="purple" borderRadius="full">{afItems.length}</Badge>}
          </Button>
      ...
      {showForm && (
        <Box bg="blue.50" border="1px" borderColor="blue.200" borderRadius="lg" p={4} mb={6}>
          ...
          <Grid templateColumns={{ base: "1fr 1fr", md: "repeat(4, 1fr)" }} gap={3} mb={3}>
            {[["Lot #","lot","26086-01"],["Description","description","BNLS Beef Brisket"],
              ["Brand","brand","IBP"],["Species","species","Beef"],["EST #","est","9268"],
              ["Pack Date","packDate","","date"],["Received Date","receivedDate","","date"],
              ["Weight (lb)","weight","0"],["Qty Cases","qtyCases","0"],["Qty Pallets","qtyPallets","0"],
            ...
            : (
              <Box overflowX="auto" mb={4}>
                <Box as="table" width="100%" borderCollapse="collapse">
                  <thead>
                    <tr>
                      <Th bg="purple.50">Lot #</Th><Th bg="purple.50">AF Description</Th><Th bg="purple.50">NTI Description</Th>
                      <Th bg="purple.50">AF Brand</Th><Th bg="purple.50">NTI Brand</Th>
                      <Th bg="purple.50">AF Weight</Th><Th bg="purple.50">NTI Weight</Th><Th bg="purple.50">Status</Th>
                    </tr>
                  </thead>
                  <tbody>
                    {discrepancies.map((row, i) => (
                      <Box as="tr" key={i} bg={row.hasIssue ? "red.50" : (i % 2 === 0 ? "white" : "gray.50")}>
                        ...
                        <Td>
                          {row.ntiOnly ? <Badge colorScheme="yellow">NTI Only</Badge>
                            : !row.matched ? <Badge colorScheme="red">Missing in NTI</Badge>
                            : row.hasIssue ? <Badge colorScheme="orange">Mismatch</Badge>
        ...
        : (
          <Box overflowX="auto">
            <Box as="table" width="100%" borderCollapse="collapse">
              <thead>
                <tr>
                  <Th>Lot #</Th><Th>Description</Th><Th>Brand</Th><Th>Species</Th>
                  <Th>EST #</Th><Th>Pack Date</Th><Th>Rcvd Date</Th>
                  <Th>Weight</Th><Th>Cases</Th><Th>Pallets</Th><Th>Notes</Th><Th></Th>
                </tr>
              </thead>
              <tbody>
